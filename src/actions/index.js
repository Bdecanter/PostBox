...
import {ACTION_TYPES_POSTS} from "./action-types"
const END_POINT = "http://localhost:3000"

export function readAllPost() {
    return function(dispatch) {
        axios.get(`${END_POINT}/posts`).then((response) => {
            dispatch({type: ACTION_TYPES_POSTS.READ_ALL, payload: response.data})
        })
    }
}
...
export function readPost(id) {
    return function(dispatch) {
        axios.get(`${END_POINT}/posts/${id}`).then((response) => {
            dispatch({type: ACTION_TYPES_POSTS.READ, payload: response.data})
        })
    }
}
...
export function createPost(post) {
    return function(dispatch) {
        axios.post(`${END_POINT}/posts`, 
        {
            ...
            dispatch({type: ACTION_TYPES_POSTS.CREATE, payload: response.data})
        })
    }
}